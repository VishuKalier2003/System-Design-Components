...
@Component("kycHandler")
public class KycHandler implements Handler {
    private Handler nextNode;
    private static final String SERVICE_CONNECTION = "kyc";
    ...
    @Override public CommandData createCommand(Data data) {
        if(prevCommandState == null) {
            ...
            return CommandData.builder().data(data).commandState(CommandState.RECEIVE).targetBusiness("kyc").build();
        }
        if(null == data.getState()) {
            if(prevCommandState == CommandState.RECEIVE) {
                prevCommandState = CommandState.EXECUTE;
                return CommandData.builder().data(data).commandState(CommandState.EXECUTE).targetBusiness("kyc").build();
            } else if(prevCommandState == CommandState.EXECUTE) {
                prevCommandState = CommandState.EXTRACT;
                return CommandData.builder().data(data).commandState(CommandState.EXTRACT).targetBusiness("kyc").build();
            }
        } else { // In case of failure of data
            switch (data.getState()) {
                case RETRY -> {
                    return CommandData.builder().data(data).commandState(prevCommandState).targetBusiness("kyc").build();
                }
                case ERROR -> {
                    prevCommandState = CommandState.COMPENSATE;
                    return CommandData.builder().data(data).commandState(CommandState.COMPENSATE).targetBusiness("kyc").build();
                }
                default -> {
                    if(prevCommandState == CommandState.RECEIVE) {
                        prevCommandState = CommandState.EXECUTE;
